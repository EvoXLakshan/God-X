#!/bin/bash
# @fileOverview Check usage stats of God-X
# @author EvoXLakshan
# @Copyright © 2025 EvoXLakshan
# @license MIT
#
# You may not reproduce or distribute this work, in whole or in part, 
# without the express written consent of the copyright owner.
#
# For more information, visit: https://github.com/EvoXLakshan/God-X

# Function to display the menu
show_menu() {
    echo "Welcome to God-X Installer/Uninstaller"
    echo "Please choose an option:"
    echo "1. Run God-X (Install)"
    echo "2. Uninstall God-X"
    echo "3. Exit"
}
...
while true; do
    ...
    case $CHOICE in
        1)
            echo "Proceeding with God-X installation..."
            break
            ;;
        2)
            echo "Uninstalling God-X..."
            bash <(curl -s https://raw.githubusercontent.com/EvoXLakshan/God-X/main/rm-TX.sh)
            echo "God-X has been uninstalled."
            exit 0
            ;;
        3)
            echo "Exiting..."
...
echo "Downloading God-X version $VERSION..."
if [ "$VERSION" == "latest" ]; then
    DOWNLOAD_URL="https://github.com/EvoXLakshan/God-X/archive/refs/heads/main.zip"
else
    DOWNLOAD_URL="https://github.com/EvoXLakshan/God-X/archive/refs/tags/$VERSION.zip"
fi

cd /home/$USERNAME
if curl -L "$DOWNLOAD_URL" -o God-X.zip; then
    echo "Download successful. Extracting files..."
    # Remove existing God-X directory if it exists
    rm -rf /home/$USERNAME/God-X
    unzip -o God-X.zip -d /home/$USERNAME
    EXTRACTED_DIR=$(ls /home/$USERNAME | grep "God-X-" | head -n 1)
    # Remove the old directory if it exists, then rename
    rm -rf "/home/$USERNAME/God-X"
    mv "/home/$USERNAME/$EXTRACTED_DIR" "/home/$USERNAME/God-X"
    rm God-X.zip
else
    echo "Failed to download God-X version $VERSION. Exiting."
    exit 1
fi
...
if [ -d "/home/$USERNAME/God-X/templates" ]; then
    ...
    if [ -f "/home/$USERNAME/God-X/templates/index.html" ] && [ -f "/home/$USERNAME/God-X/templates/result.html" ]; then
        echo "Template files verified."
    else
        echo "Warning: Some template files are missing."
        echo "Files in templates directory:"
        ls -la "/home/$USERNAME/God-X/templates/"
    fi
else
    echo "Error: Templates directory not found after extraction."
    echo "Contents of God-X directory:"
...
mkdir -p /var/lib/God-X/certs
sudo chown -R $USERNAME:$USERNAME /var/lib/God-X/certs

# Check if valid certificate already exists
if [ -f "/var/lib/God-X/certs/$DOMAIN.cer" ] && [ -f "/var/lib/God-X/certs/$DOMAIN.cer.key" ]; then
    echo "Valid SSL certificate already exists."
    SSL_CONTEXT="--certfile=/var/lib/God-X/certs/$DOMAIN.cer --keyfile=/var/lib/God-X/certs/$DOMAIN.cer.key"
else
    echo "Generating SSL certificate..."
    curl https://get.acme.sh | sh -s email=$USERNAME@$SERVER_IP
    ~/.acme.sh/acme.sh --issue --force --standalone -d "$DOMAIN" \
        --fullchain-file "/var/lib/God-X/certs/$DOMAIN.cer" \
        --key-file "/var/lib/God-X/certs/$DOMAIN.cer.key"
    # Fix ownership of the generated certificates
    sudo chown $USERNAME:$USERNAME /var/lib/God-X/certs/$DOMAIN.cer
    sudo chown $USERNAME:$USERNAME /var/lib/God-X/certs/$DOMAIN.cer.key
...
if sudo systemctl is-active --quiet god-x; then
    echo "Stopping existing God-X service..."
    sudo systemctl stop god-x
fi

# Create a systemd service to keep the Flask app running with Gunicorn
echo "Setting up systemd service..."
cat > /etc/systemd/system/god-x.service <<EOL
[Unit]
Description=God-X Web App
After=network.target

[Service]
User=$USERNAME
WorkingDirectory=/home/$USERNAME/God-X
ExecStart=/bin/bash -c 'source /home/$USERNAME/God-X/venv/bin/activate && exec gunicorn -w 4 -b 0.0.0.0:$PORT $SSL_CONTEXT app:app'
Environment="DB_PATH=/etc/x-ui/x-ui.db"
Restart=always
RestartSec=5
StandardOutput=append:/var/log/god-x.log
StandardError=append:/var/log/god-x.log
SyslogIdentifier=god-x
...
sudo systemctl enable god-x
sudo systemctl start god-x
